REGEXP = /([a-z]+)\s(dec|inc)\s(-?\d+)\sif\s([a-z]+)\s(!=|<=|>=|==|<|>)\s(-?\d+)/i.freeze

# returns the max value seen during the execution of instructions
def registers_maxval(instructions)
  r = Hash.new(0) # registers (zero by default)
  m = 0

  # parse and process instructions
  instructions.split("\n").each do |instruction|
    v = instruction.match(REGEXP)[1..-1] # 0 - original (input) string
    n = v[2].to_i
    next if n.zero?

    z = v.last(3) # parse condition
    z[0] = "r['#{z[0]}']" # get the register value
    z[-1] = z[-1].to_i # str -> int
    next unless eval(z.join) # condition met?
