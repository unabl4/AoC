def closest_particle(particles)
  # p,v,a 
  # no need to iterate; analytic solution is available
  1_000_00.times do
    # main logic
  ...
  # select the closest one (distance-wise)
  particles.each_with_index.min_by { |particle,_| particle[0].abs + particle[1].abs + particle[2].abs }[1]
end

def parse(raw_particles)
  regex = /p=<(-?\d+),(-?\d+),(-?\d+)>,\sv=<(-?\d+),(-?\d+),(-?\d+)>,\sa=<(-?\d+),(-?\d+),(-?\d+)>/i
  
  raw_particles.map do |raw_particle|
    raw_particle.match(regex)[1..-1].map(&:to_i)
  end
end
