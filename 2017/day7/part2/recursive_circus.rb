class Node 
  attr_accessor :name, :prev, :leaves
  ...
  # get weight (recursive)
  def weight
    leaves_weight = leaves.map(&:weight).reduce(:+) || 0
    @weight + leaves_weight
  end
  
  def balanced?
    leaves.map(&:weight).uniq.size == 1
  end
...
def find_weight(input)
  ...
  i.each do |x|
    ...
    t[1].split(', ').each do |v|
      ...
      nodes[m[1]].leaves << nodes[v]
    end
  end
  
  # step 3: track back to the root
  r = nodes.values.sample # random
  r = r.prev while r.prev # <- root
