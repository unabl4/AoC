i = "una bokpr ftz ryw nau yknf fguaczl anu
...
def is_valid?(password)
  groups = password.split.group_by(&:itself)
  words = groups.keys # unique words
  return false unless groups.all? { |_,v| v.size == 1 } # required condition
  
  # anagram check
  h = {}
  words.each do |w|
    v = w.chars.sort.join # sorted
    return false if h[v]
    h[v] = true # add
  end
